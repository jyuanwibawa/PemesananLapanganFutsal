<section class="content-section">
    ...
    <form action="{{ route('admin.lapangan.update', $lapangan->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <textarea name="deskripsi" id="deskripsi" rows="4">{{ old('deskripsi', $lapangan->deskripsi) }}</textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="status_aktif" id="status_aktif" required>
                <option value="1" {{ old('status_aktif', $lapangan->status_aktif) == 1 ? 'selected' : '' }}>Aktif
                </option>
                <option value="0" {{ old('status_aktif', $lapangan->status_aktif) == 0 ? 'selected' : '' }}>Nonaktif
                </option>
            </select>
        </div>
